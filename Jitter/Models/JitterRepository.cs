...
namespace Jitter.Models
{
    public class JitterRepository
    {
        ...
        {
            //select * from JitterUsers as Users where users.FirstName like %search_term%
        ...
        public bool CreateJot(JitterUser a_jitter_user, string content)
        {
            Jot a_jot = new Jot { Content = content, Date = DateTime.Now, Author = a_jitter_user };
            bool is_added = true;
            try
            {
                Jot added_jot = _context.Jots.Add(a_jot);
                _context.SaveChanges();
                /* Why is this null? Docs?
                if (added_jot == null)
                {
                    is_added = false;
                }*/
